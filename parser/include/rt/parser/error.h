...
/*   Updated: 2024/10/10 23:45:11 by kiroussa         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
typedef struct s_rt_parser_error
{
	enum e_rt_parser_error	type;
	union
	{
		void						*data;
...
t_rt_parser_error	rt_parse_ok(void);
t_rt_parser_error	rt_parse_err(enum e_rt_parser_error type);
t_rt_parser_error	rt_parse_errd(enum e_rt_parser_error type, void *data);
t_rt_parser_error	rt_parse_errs(enum e_rt_parser_error type,
						const char *string_data);
t_rt_parser_error	rt_parse_err_file(t_rt_parser_file_context context);

const char			*rt_parse_err_str(t_rt_parser_error err);
void				rt_parse_err_print(t_rt_parser parser,
						t_rt_parser_error err);
